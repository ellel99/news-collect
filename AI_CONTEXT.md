...
- 当前 Active SPEC：`spec/SPEC-0025-provider-adapter-scaffold.md`
- 最近完成：SPEC-0003，tag `spec-0003-completed`
- 当前工作状态：SPEC-0025 Provider Adapter Scaffold Implementation Review；只允许 provider
  contracts、fail-closed registry、mock transport 与 Marketaux mocked scaffold
...
- SPEC-0019 pure contract、SPEC-0020 pure mapping scaffold、SPEC-0021 Docs Review/schema
  implementation、SPEC-0023 Docs Review/implementation 与 SPEC-0024 Docs Review 均已 Completed；
  当前只实现 mock-only Provider Adapter scaffold，不得修改 migration/ORM/schema、请求 Provider、
  读取 `.env`/local capture，或接入 collection/正式 normalization/dedup/Event/AI
...
- SPEC-0025 只实现 Marketaux mocked scaffold；Finnhub/EIA/SEC real adapters、NewsAPI.ai/GDELT 均
  不激活。SPEC-0022 仍为非 Active Dedup/Event candidate，SPEC-0005 X Source 范围不变
